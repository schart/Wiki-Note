...
import * as note_queries from "../../DB/database.note/note.create";
import * as note_auth_queries from "../../DB/database.note/note.auth";
import * as note_delete_queries from "../../DB/database.note/note.delete"
import * as comment_queries from "../../DB/database.comment/comment.create"
...
class Note 
{
    note_upload: any = async (req: Request, res: Response, next: NextFunction) => 
    {
        ...
        uploads(req, res, async (error: Error): Promise<any> => 
        {
            ...
            else 
            {
                ...
                file_name = req.file?.filename; body = req.body; title = body.title; url = body.url
                ...
                await note_queries.Note_save(token.Id, [title, url, file_name])
                    .then((result: any) => 
                    {
    ...
    note_accept: any = async (req: Request, res: Response, next: NextFunction) => //! for admin
    {
        let token: any; token = JSON.parse(JSON.stringify(jwtDecode(req.cookies.token)));

        await note_auth_queries.Note_accept(/*token.Id,*/ req.params.noteId)
    ...
    note_delete: any = async (req: Request, res: Response, next: NextFunction) => //! for admin
    {
        let token: any; token = JSON.parse(JSON.stringify(jwtDecode(req.cookies.token)));

        await note_delete_queries.Note_delete(req.params.noteId, token.Id)
            .then((result: any) => { if (result == true) return res.status(200).json({ ok: true, msg: "has been deleted" }) })
            .catch((error: Error) => { return res.status(400).json({ ok: false, error, msg: "there are problems please try again at later" }) })
    }


    note_comment: any = async (req: Request, res: Response, next: NextFunction) => //! for public
    {
        let token: any; token = JSON.parse(JSON.stringify(jwtDecode(req.cookies.token)));

        await comment_queries.Note_comment(token.Id, req.body.noteId, req.body.comment)
            .then((result: any) => { if (result == true) return res.status(200).json({ ok: true, msg: "has been shared" }) })
            .catch((error: Error) => { return res.status(400).json({ ok: false, error, msg: "there are problems please try again at later" }) })
    }


    note_like: any = async (req: Request, res: Response, next: NextFunction) => //! for public 
    {   
        let token: any; token = JSON.parse(JSON.stringify(jwtDecode(req.cookies.token)));

        await note_queries.Note_like(token.Id, req.body.noteId) 
            .then((result: any) => { if (result == true) return res.status(200).json({ ok: true, msg: "has been liked" }) })
            .catch((error: Error) => { return res.status(400).json({ ok: false, error, msg: "there are problems please try again at later" }) })
